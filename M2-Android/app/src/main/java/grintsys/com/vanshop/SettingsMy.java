...
public class SettingsMy {
    ...
    public static final String PREF_CLIENT_NAME_SELECTED = "pref_client_name_selected";

    public static final String SENT_TOKEN_TO_SERVER = "sentTokenToServer";
    public static final String REGISTRATION_COMPLETE = "registrationComplete";

    private static final String TAG = SettingsMy.class.getSimpleName();
    private static Tenant actualTenant;
     ...
     */
    public static Tenant getActualTenant() {
        if (actualTenant != null) {
            ...
            if (json.isEmpty() || "null".equals(json)) {
                ...
                actualTenant = Utils.getGsonParser().fromJson(json, Tenant.class);
                Timber.d("%s - Returned shop from memory: %s", TAG, actualTenant.toString());
                return actualTenant;
            }
        }
    }
     ...
     *
     * @param actualTenant selected shop or null for disable selection.
     */
    public static void setActualTenant(Tenant actualTenant) {
        if (actualTenant != null)
            Timber.d("%s - Set selected shop: %s", TAG, actualTenant.toString());
        else
            Timber.d("%s - Disable selected shop", TAG);
        SettingsMy.actualTenant = actualTenant;

        String json = Utils.getGsonParser().toJson(SettingsMy.actualTenant);
     ...
     */
    @NonNull
    public static Tenant getActualNonNullShop(Activity activity) {
        Tenant tenant = getActualTenant();
        if (tenant == null) {
            if (activity != null) {
                ...
                Timber.e("Null tenant detected also with null activity parameter.");
            }
            // Return an empty tenant to prevent a null pointer exception before intent is processed.
            return new Tenant();
        } else {
            return tenant;
        }
    ...
    public static void setActiveUser(User user) {
        ...
        //String token = SettingsMy.activeUser.getAccessToken();
