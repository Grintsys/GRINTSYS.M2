...
import grintsys.com.vanshop.entities.delivery.Shipping;
...
public class Analytics {
     ...
     *
     * @param tenant    tenant with app specific Google Ua or null, if global tracker is enough.
     * @param context application context.
     */
    public static synchronized void prepareTrackersAndFbLogger(Tenant tenant, Context context) {
        ...
        if (tenant == null) {
            ...
            if (!mTrackers.containsKey(TRACKER_APP) && analytics != null) {
                if (tenant.getGoogleUa() != null && !tenant.getGoogleUa().isEmpty()) {
                    Timber.d("Set new app tracker with id: %s", tenant.getGoogleUa());
                    // App tracker determined by tenant
                    Tracker appTracker = analytics.newTracker(tenant.getGoogleUa());
     ...
     *
     * @param actualNonNullTenant active shop before change.
     * @param newTenantSelected   active shop after change.
     */
    public static void logShopChange(Tenant actualNonNullTenant, Tenant newTenantSelected) {
        if (actualNonNullTenant != null && newTenantSelected != null) {
            String description = "From (id=" + actualNonNullTenant.getId() + ",name=" + actualNonNullTenant.getName() +
                    ") to (id=" + newTenantSelected.getId() + ",name=" + newTenantSelected.getId() + ")";
            // FB facebookLogger
            Bundle parameters = new Bundle();
