...
public class MsgUtils {
    ...
    public static void showMessage(Activity activity, JSONObject message) {
        try {
            String msj = message.getJSONObject("error").getString("message");
            String details = message.getJSONObject("error").getString("details");
            Timber.e("Error message: %s, %s", msj, details);
            if (activity != null)
                MsgUtils.showToast(activity, TOAST_TYPE_MESSAGE, String.format("%s, %s", msj, details), ToastLength.LONG);
        } catch (Exception e) {
            Timber.e(e, "ShowMessage exception");
            if (activity != null)
                MsgUtils.showToast(activity, TOAST_TYPE_INTERNAL_ERROR, "ShowMessage exception", ToastLength.SHORT);
        }
    }
