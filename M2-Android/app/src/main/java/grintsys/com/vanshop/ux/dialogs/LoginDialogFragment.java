...
import grintsys.com.vanshop.listeners.OnTouchPasswordListener;
...
public class LoginDialogFragment extends DialogFragment implements FacebookCallback<LoginResult> {
    ...
    private void logInWithEmail(EditText editTextEmail, EditText editTextPassword) {
        ...
        JSONObject jo;
        try {
            jo = JsonUtils.createUserAuthentication(username,
                    password,
                    SettingsMy.getActualNonNullShop(getActivity()).getId());
            editTextPassword.setText("");
        } catch (JSONException e) {
            String message = "Parse logInWithEmail exception";
            Timber.e(e, message);
        ...
        GsonRequest<UserResult> userLoginEmailRequest = new GsonRequest<>(Request.Method.POST, EndPoints.USER_LOGIN_EMAIL, jo.toString(), UserResult.class,
                new Response.Listener<UserResult>() {
                    ...
                    public void onResponse(@NonNull UserResult response) {
                        ...
                        handleUserLogin(response.result);
                    }
    ...
    private void resetPassword(EditText emailOfForgottenPassword) {
        String url = String.format(SettingsMy.getActualTenant().getUrl() + EndPoints.USER_RESET_PASSWORD, SettingsMy.getActualNonNullShop(getActivity()).getId());
        progressDialog.show();
