...
public class AccountEditFragment extends Fragment {
    ...
    private void putUser(User user) {
        if (isRequiredFields()) {
            ...
            if (activeUser != null) {
                ...
                String url = String.format(EndPoints.USER_SINGLE, activeUser.getId());

                progressDialog.show();
                GsonRequest<UserResult> req = new GsonRequest<>(Request.Method.PUT, url, joUser.toString(), UserResult.class,
                        new Response.Listener<UserResult>() {
                            @Override
                            public void onResponse(@NonNull UserResult result) {
                                SettingsMy.setActiveUser(result.result);
                                refreshScreen(result.result);
    ...
    private void changePassword() {
        if (isRequiredPasswordFields()) {
            ...
            if (user != null) {
                String url = String.format(SettingsMy.getActualTenant().getUrl() + EndPoints.USER_CHANGE_PASSWORD, SettingsMy.getActualNonNullShop(getActivity()).getId(), user.getId());
