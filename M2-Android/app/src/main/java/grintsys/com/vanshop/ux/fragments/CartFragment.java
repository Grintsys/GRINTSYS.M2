            ...
            GsonRequest<CartResult> getCart = new GsonRequest<>(Request.Method.GET, url, null, CartResult.class,
                    new Response.Listener<CartResult>() {
                        ...
                        public void onResponse(@NonNull CartResult cart) {
                            ...
                            if (cart.result.getItems() == null || cart.result.getItems().size() == 0) {
    ...
    private void prepareCartRecycler(View view) {
        ...
        cartRecyclerAdapter = new CartRecyclerAdapter(getActivity(), new CartRecyclerInterface() {
            ...
            private void deleteItemFromCart(final long id, boolean isDiscount) {
                User user = SettingsMy.getActiveUser();
                if (user != null) {
                    String url = String.format(EndPoints.CART_ITEM_DELETE, id);
                    progressDialog.show();

                    GsonRequest<Result> req = new GsonRequest<>(Request.Method.DELETE, url, null, Result.class,
