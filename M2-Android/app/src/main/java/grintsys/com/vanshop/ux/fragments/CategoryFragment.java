...
import grintsys.com.vanshop.entities.product.ProductListResult;
import grintsys.com.vanshop.interfaces.CategoryRecyclerInterface;
...
public class CategoryFragment extends Fragment {
    ...
    private void getProducts(String url) {
        ...
        if (url == null) {
            ...
            {
                long tenantId = SettingsMy.getActualNonNullShop(getActivity()).getId();
                Timber.d("CurrentTenant: %d", tenantId);
            ...
            if (sortItem != null) {
                url = url + "&sorting=" + sortItem.getValue();
            }
        }
        ...
        GsonRequest<ProductListResult> getProductsRequest = new GsonRequest<>(Request.Method.GET, url, null, ProductListResult.class,
                new Response.Listener<ProductListResult>() {
                    @Override
                    public void onResponse(@NonNull ProductListResult response) {
                        firstTimeSort = false;
//                        Timber.d("response:" + response.toString());
                        productsRecyclerAdapter.addProducts(response.result.getProducts());
                        productsMetadata = response.result.getMetadata();
        ...
        getProductsRequest.setShouldCache(true);
