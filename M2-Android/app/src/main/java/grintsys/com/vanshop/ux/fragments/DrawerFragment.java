...
import grintsys.com.vanshop.entities.drawerMenu.DrawerResult;
import grintsys.com.vanshop.interfaces.DrawerRecyclerInterface;
import grintsys.com.vanshop.interfaces.DrawerSubmenuRecyclerInterface;
import grintsys.com.vanshop.utils.MsgUtils;
import grintsys.com.vanshop.ux.MainActivity;
import grintsys.com.vanshop.ux.adapters.DrawerRecyclerAdapter;
import grintsys.com.vanshop.ux.adapters.DrawerSubmenuRecyclerAdapter;
...
//                toolbar.setAlpha(1 - slideOffset / 2);
    ...
    getDrawerItems() {
        ...
        //String accessToken = SettingsMy.getActiveUser().getAccessToken();
        GsonRequest<DrawerResult> getDrawerMenu = new GsonRequest<>(Request.Method.GET, EndPoints.CATEGORIES, null, DrawerResult.class,
                new Response.Listener<DrawerResult>() {
            @Override
            public void onResponse(@NonNull DrawerResult drawerResponse) {
                drawerRecyclerAdapter.addDrawerItem(new DrawerItemCategory(BANNERS_ID, BANNERS_ID, getString(R.string.Just_arrived)));
                drawerRecyclerAdapter.addDrawerItemList(drawerResponse.result.getNavigation());
                drawerRecyclerAdapter.addPageItemList(drawerResponse.result.getPages());
                drawerRecyclerAdapter.notifyDataSetChanged();

                if (drawerListener != null)
                    drawerListener.prepareSearchSuggestions(drawerResponse.result.getNavigation());
