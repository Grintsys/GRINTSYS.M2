    ...
    private void loadPayments(String url) {
        ...
        if (user != null && tenant != null) {
            ...
            GsonRequest<PaymentListResult> req = new GsonRequest<>(Request.Method.GET, url, null, PaymentListResult.class,
                    new Response.Listener<PaymentListResult>() {
                    @Override
                    public void onResponse(PaymentListResult response) {
                        //paymentsMetadata = response.getMetadata();
                        paymentsHistoryRecyclerAdapter.addPayments(response.result.getPayments());
